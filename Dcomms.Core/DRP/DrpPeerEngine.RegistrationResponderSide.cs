...
namespace Dcomms.DRP
{
    partial class DrpPeerEngine
    {
        ...
        internal async Task AcceptRegisterRequestAsync(LocalDrpPeer acceptAt, RoutedRequest routedRequest) // engine thread
        {
            ...
            {
                if (req.AllowConnectionsToRequesterRegistrationId == false) throw new InvalidOperationException("same reg. id 43717");
                else WriteToLog_reg_responderSide_higherLevelDetail($"accepting reg. request from same registration ID");
            }
            ...
            try
            {
                ...
                try
                {
                    ...
                    newConnectionToNeighbor.InitialPendingPingRequest = new PendingLowLevelUdpRequest("pendingPingRequest 693", newConnectionToNeighbor.RemoteEndpoint,
                                    PongPacket.GetScanner(newConnectionToNeighbor.LocalNeighborToken32, ping.PingRequestId32), PreciseDateTimeNowUtc,
